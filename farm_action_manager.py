...
from farm_models import Factory, Animal, Greenhouse
...
class FarmActionManager:
    ...
    def _determine_animal_product(self, animal_proto: str) -> str:
        """Accurately maps specific animal prototype names to their real game items."""
        proto_lower = animal_proto.lower()
        
        # Birds / Plumed
        if "chicken" in proto_lower or "goose" in proto_lower or "gander" in proto_lower:
            return "CR_EGG"
        # Hoofed / Mammals
        elif "cow" in proto_lower:
            return "CR_MILK"
        elif "sheep" in proto_lower:
            return "CR_WOOL"
            
        return "CR_MILK" # Safe default fallback


    def collect_from_animals_mass(self, animals_list: list[Animal]) -> dict:
        """Collects ALL available items from materials arrays for a batch of animals."""
        if not animals_list: return {}
        events = []
        
        for animal in animals_list:
            # Iterate through EVERY ready product inside the animal's materials array
            for mat in animal.materials:
                raw_item = mat.get("item", "")
                item_id = raw_item[1:] if raw_item.startswith("@") else raw_item
                if not item_id:
                    item_id = self._determine_animal_product(animal.item_proto)
        ...
        print(f"[ActionManager]: Posting mass collection containing {len(events)} sub-events for animals...")
        ...
        if response and response.get("cmd") != "ERR":
            for animal in animals_list:
                ...
                animal.materials = [] # Fully empty storage on success
    ...
    def collect_from_factories_mass(self, factories_list: list[Factory], repeat_on_pick: bool = False) -> dict:
        """Collects ALL available items from factories and optionally restarts production using current workers."""
        if not factories_list: return {}
        events = []
        
        for factory in factories_list:
            # 1. Generate collection sub-events for EVERY item inside materials
            ...
            if repeat_on_pick and factory.workers and factory.current_craft:
                recipe_id = factory.current_craft.get("id", "")
                if not recipe_id.startswith("U_") and not recipe_id.startswith("@"):
                    recipe_id = f"U_{recipe_id}"
                elif recipe_id.startswith("@"):
    ...
    def start_craft_in_factories_mass(self, factories_list: list[Factory], recipe_id: str) -> dict:
        """Mass launches a craft recipe ONLY for working factories with available queue slots."""
        if not factories_list: return {}
        events = []
        
        formatted_recipe = recipe_id if recipe_id.startswith("U_") else f"U_{recipe_id}"
        
        for factory in factories_list:
            if not factory.workers or not factory.current_craft:
                print(f"[ActionManager]: Skipping factory ID {factory.id} ({factory.item_proto}) - No hired workers found!")
                continue
                
            current_active_slots = 0
            if factory.current_craft:
        ...
        print(f"[ActionManager]: Posting mass manual craft '{formatted_recipe}' for {len(events)} valid factories...")
        response = self.client.execute_raw_action(events)
        self.data_manager.update_from_server_response(response)
        return response
    ...
    def harvest_greenhouses_mass(self, greenhouses_list: list[Greenhouse]) -> dict:
        ...
        for gh in greenhouses_list:
            ...
            crop_id = f"P_{crop_name}" if crop_name else "P_WHEAT"
